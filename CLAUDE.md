...
- `ARCHITECTURE.md` is the source of truth for current architecture (precedence: `ARCHITECTURE.md` > the active plan and its deviation log; superseded specs and plans are deleted — git history is the archive). When an architecture decision is made or changed, update `ARCHITECTURE.md` (the what and why) in the same change; the plan's deviation log records why the change happened. Keep it current.
...
- One source for current state. The active plan's deviation log is the canonical record of what's actually happened; `ARCHITECTURE.md`'s status line is the one-glance summary. Don't duplicate "where we are" content elsewhere (parallel status text in a separate doc has historically lagged real status by multiple phases).
...
Seven docs persist long-term: README.md, CLAUDE.md, AGENTS.md, ARCHITECTURE.md, tokens/README.md, docs/workflow.md, docs/component-conventions.md. The rules below govern all of them.
...
When touching any of the persistent docs, re-read the others end-to-end before committing. Drift hides in lines you don't think to look at — the reviewer subagent catches code drift; this re-read catches doc drift.
...
- Gate suite (single copy — the skill and workflow doc point here; CI's `correctness` job runs the same list): `pnpm build && pnpm typecheck && pnpm lint && pnpm knip && pnpm spell && pnpm format && pnpm test && pnpm smoke:dist && pnpm verify:pack && pnpm verify:types && pnpm assert:use-client`
- `pnpm chromatic` — visual gate (normally runs in CI)
...
- tsdown cleans its `dist/` output dir each build silently. Any separate writer to `dist/` — for example the `css` step writing `dist/styles.css` — must run after tsdown in the `build` script (`… && tsdown && pnpm css`), or tsdown deletes it and the build still "succeeds" with the file missing.
- A `*/` inside a CSS block comment terminates it early — writing variable globs like `--color-*/--spacing-*` in a comment closes the comment at `-*/` and the rest becomes invalid CSS. Lightning CSS then panics with a cryptic `ParserError(EndOfInput)` at end-of-file, far from the actual comment. Write `--color-*, --spacing-*` instead.
- Storybook serves a boot-time copy of `dist/styles.css` — rebuilding it does not reliably hot-reload (the build recreates the file, which can drop Vite's file watch). After any `pnpm build`/`pnpm css`, restart Storybook and confirm the old process actually exited: a "Port 6006 is not available" prompt means it didn't, and every check will silently pass against stale styles. Symptom: token changes invisible, dark toggle "stops working" because the new variables don't exist in the served copy.
