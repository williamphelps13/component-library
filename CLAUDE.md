...
- Dependabot can't regenerate a pnpm-`catalog:` lockfile. When a grouped bump touches the catalog in `pnpm-workspace.yaml`, it writes the resolved version as the lockfile specifier instead of `catalog:`, so CI fails `--frozen-lockfile` (`ERR_PNPM_OUTDATED_LOCKFILE … react (lockfile: 19.2.7, manifest: catalog:)`) — reads like a project bug, isn't. Fix: check out the branch, `pnpm install`, push; `@dependabot recreate` reproduces the drift. Pushing to the branch can make Dependabot close the PR, so open a fresh PR from the corrected branch if it does.
- npm OIDC publish with provenance requires `package.json` `repository.url`. Without it the publish fails late with a cryptic `E422 … Error verifying sigstore provenance bundle … "repository.url" is ""` — looks like a registry bug, isn't. A token publish silently ships _without_ provenance, so the gap only surfaces the first time OIDC (with `publishConfig.provenance: true`) is exercised. Fix: add a `repository` field whose URL matches the GitHub repo. `publint` does not flag the missing field as fatal, so nothing catches it pre-publish.
- The Changesets-bot "Version Packages" PR can't satisfy branch protection on its own. It's created with `GITHUB_TOKEN`, and GitHub deliberately doesn't trigger workflows from `GITHUB_TOKEN` events — so `ci.yml` never runs (the run sits at `action_required`, 0s), the required `correctness`/`chromatic` checks never report, and the merge is blocked (no admin bypass under `enforce_admins`). Fix: `gh pr close <n> && gh pr reopen <n>` re-fires CI as a real user. Recurs on every release.
