...
- Shipped CSS must live in cascade layers named `theme/base/components/utilities` (matching Tailwind v4), not a private layer. Same-named layers merge across stylesheets, so a consumer's Preflight stays below our components and their utilities stay above — in either import order. A private layer name silently fails one way or the other (Preflight strips button styles, or consumer utility overrides stop working); only a real two-stylesheet render catches it.
