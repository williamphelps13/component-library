...
- **Phase 4 (Button) pulled forward to verify the harness — IN PROGRESS (see `HANDOFF.md`).** The empty harness was a hollow verification (B2/B3 can't be verified without a component), so Phase 4 was pulled forward. Validated vs current docs: **React 19 `ref` is a plain prop** — `button.tsx` is a plain function, `ref?: Ref<HTMLButtonElement>` + explicit `ReactElement` return (isolatedDeclarations), **no `forwardRef`**. **CSF Next `play` uses context args** (`{ canvas, userEvent, args }` + `import { fn, expect } from 'storybook/test'`), not `within(canvasElement)`. **Vitest `unit` project = `environment: 'node'`** (pure-logic; no jsdom dep). **Dynamic-class-names bug (caught by the real component):** `ui-btn-${intent}` can't be statically resolved by Tailwind, so the precompiled CSS silently omitted `ui-btn-neutral`; fixed with typed literal-class `Record<Intent|Size, string>` maps in `variants.ts` (all variants ship + TS enforces completeness; CLAUDE.md gotcha added). **tsconfig LAYERED (replaces an earlier reflexive "exclude stories" that lost story type-checking):** `tsconfig.build.json` (strict — isolatedDeclarations, rootDir:src, src-only, exclude stories/tests) used by tsdown (`--tsconfig`) + typecheck pass 1; `tsconfig.json` broad (`src` + `.storybook/**/*`, lenient) for editor + typecheck pass 2 → stories/preview/`main.ts` + ambient `globals.d.ts` ARE type-checked. `pnpm typecheck` runs both. (Originally written bare `.storybook`, which silently matched nothing — see BLOCKER below.) **Verified so far:** variants TDD green; `pnpm build` + `assert:use-client` + build-config typecheck + publint green; `dist/styles.css` ships all 6 `ui-btn*` variants incl `neutral`. **✅ BLOCKER RESOLVED (2026-05-25) — root cause was the `include` glob, none of the three original hypotheses.** Symptom: `pnpm typecheck` pass 2 fails `TS2882: Cannot find module or type declarations for side-effect import of '../dist/styles.css'` in `.storybook/preview.tsx`. Diagnosis by *evidence*, not guesswork: `tsc --showConfig` showed the broad config's resolved `files` array held **only `src/**`** — zero `.storybook/*` files — despite `include: ['src', '.storybook']`. **TS's `include` globber silently skips dot-directories**, so bare `.storybook` matched nothing; `preview.tsx` was in the program only because a story imports it, and `globals.d.ts` (imported by nobody) never loaded → its `declare module '*.css'` wildcard never registered. (The TS2882 *check itself* is a TS 6.0 change: `noUncheckedSideEffectImports` now defaults on — side-effect imports are verified where pre-6.0 they were silently ignored.) The three hypotheses (globals-not-in-include in the naive sense / `verbatimModuleSyntax` / real-file shadowing) and the "drop `verbatimModuleSyntax`" fix were **all wrong** — disproved by `--listFilesOnly` (globals.d.ts absent from the program) and a throwaway-config typecheck (corrected include + the *existing* no-body declaration → EXIT 0; no `{}` needed, no real-file shadowing). **Fix (one line):** `tsconfig.json` → `include: ['src', '.storybook/**/*']`; as a bonus, `main.ts` is now genuinely type-checked (the layered-tsconfig note above had *claimed* it already was — it wasn't). CLAUDE.md gotcha + `ARCHITECTURE.md` §3 (TypeScript) updated. **Verified green:** `pnpm typecheck` (both passes) · `pnpm build` (10 files + `dist/styles.css`) · `pnpm test` (2 projects, 7 tests — `unit` node-env + `storybook` browser/Chromium incl. the `Clicks` play + a11y). **Visual verification (done, Playwright-driven via the MCP):** walked 4 URL-globals states on `iframe.html?id=components-button--primary` — default → `theme:dark` → `palette:brand` → `theme:dark;palette:brand` — and asserted computed `bgColor` against expectation each time (hard evidence, not just eyeballing). All four passed; case 4 is the strongest, confirming the consumer's `:root --color-primary` override beats even the dark-mode semantic token. **MCP `get-documentation`** returns the real Button props (`intent?: Intent = primary`, `size?: Size = md`, `ref?: Ref<HTMLButtonElement>` with the React-19 JSDoc preserved) — the agent-verify-via-MCP contract is proven. **Visual verification surfaced one Phase-4 harness bug:** `@storybook/addon-themes` must be registered in `definePreview.addons: [addonThemes()]`, not _only_ in `main.ts addons`. CSF Next splits manager-side addon registration (main.ts) from preview-side wiring (`definePreview.addons`); without the preview-side factory, the addon's globalType for `theme` is never registered and `withThemeByDataAttribute` silently falls back to `defaultTheme: 'light'` every render — tests/typecheck/lint all stay green because they don't assert theme behaviour. Fix landed in `.storybook/preview.tsx` (kept `withThemeByDataAttribute` as a named import — it _is_ a named export in 10.4.1, contrary to the v10.2.9 docs example that uses `addonThemes.withThemeByClassName`); CLAUDE.md gotcha added. **Reviewer-subagent checkpoint surfaced one Critical the implementer missed + drove a token fix.** Independent code-review subagent (`general-purpose`) on the WIP diff caught that the same trap the addon-themes fix addressed had an unfixed sibling in **`@storybook/addon-a11y`** — `parameters.a11y.test: 'error'` was a silent no-op because the factory wasn't in `definePreview.addons`. Reviewer empirically proved it by seeding a `button-name` violation that passed all 7 tests; on my repro the seeded violation now correctly fails. **Audit of all 5 factory-shaped addons in `main.ts`:** added `addonA11y()` + `addonChromatic()` (foundation-prep for B3) alongside the existing `addonDocs()` and the session's `addonThemes()`. **`addonVitest()` is intentionally NOT registered** in `definePreview.addons` — its module imports `vitest` at load time, which is only resolvable inside the vitest run context; placing it there crashes Storybook dev (and any non-vitest preview consumer) on import. The vitest integration is wired through the `storybookTest()` plugin in `vitest.config.ts` instead — different addons need different registration paths. CLAUDE.md gotcha was rewritten to cover the general rule + this exception. **Now-live a11y gate immediately caught a real WCAG AA violation in the primary token** (white-on-blue.500 = 3.68 : 1, AA requires 4.5 : 1). Root cause was the token, not the Button — the gate was working correctly the moment it became live. **Token fix (option 1 / "Add blue.600 + repoint primary"):** added a darker primitive `color.blue.600: oklch(0.50 0.19 256)` to `tokens/tokens.json`; repointed `light.color.primary` from `{color.blue.500}` to `{color.blue.600}`. Dark theme unchanged (blue.300 + dark text already passed). `--color-ring` deliberately left on `blue.500` (the focus-ring is a separate semantic decision; doesn't ride the same contrast contract as a CTA background). Rebuild + re-test green; all 7 tests pass. **Pending:** verify `--color-blue-600` lands in shipped `dist/styles.css`; commit `chore(phase4): Button` (+ `chore(phase2): primary-token contrast` if you want clean phase separation); delete `HANDOFF.md`; Chromatic (B3) → merged Phase 3+4 quiz gate → **over-strict-settings audit** (does `verbatimModuleSyntax` earn its friction? — note it was **not** the culprit for the include-glob blocker either, so judge it on its own merits).
